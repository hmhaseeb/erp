...
class Returns extends Component
{
    ...
    public $return_number, $return_date, $supplier_id, $purchase_id, $account_id, $return_reason;
    ...
    {
        $this->reset(['search', 'supplier_id_filter', 'date_from', 'date_to']);
        $this->perPage = 10;
        $this->sortField = 'id';
    ...
    public function mount()
    {
        ...
        if ($firstProd) {
            ...
            $this->unit_price = $firstProd->purchase_price;
    ...
    public function updatedProductId($val)
    {
        ...
        if ($prod) {
            $this->unit_price = $prod->purchase_price;
    ...
    public function saveReturn(ReturnService $returnService)
    {
        ...
        $returnService->processPurchaseReturn($header, $items);

        session()->flash('success', "Purchase Return #{$this->return_number} processed successfully.");
        $this->closeModal();
    }

    public function render()
    {
        $query = PurchaseReturn::with(['supplier', 'items.product', 'account']);
        ...
        return view('livewire.purchases.returns', [
            'suppliers' => $suppliers,
            'products' => $products,
            'accounts' => $accounts,
            'returns' => $returns,
        ])->layout('layouts.app', ['title' => 'Purchase Returns']);
    }
}
