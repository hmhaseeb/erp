                        ...
                        <div class="col-12 col-sm-6 col-lg-12 col-xl-3">
                            ...
                            <x-searchable-select wire:model="account_id" class="form-select {{ $errors->has('account_id') ? 'is-invalid' : '' }}" placeholder="Select Account...">
                                @foreach($accounts as $acc)
                                    <option value="{{ $acc->id }}">{{ $acc->name }} ({{ $acc->type }}) - {{ currency() }} {{ number_format($acc->current_balance, 2) }}</option>
                                @endforeach
                            </x-searchable-select>
                        ...
                        <tbody>
                            @foreach($items as $index => $item)
                                <tr wire:key="sale-item-{{ $index }}" class="sale-item-row">
                                    ...
                                    <td class="col-price">
                                        <label class="form-label font-size-11 text-muted d-md-none mb-1">Price ({{ currency() }}) <span class="text-danger">*</span></label>
                    ...
                    <div class="col-12 col-lg-6">
                        <div class="bg-light p-3 rounded">
                            ...
                            <div class="d-flex justify-content-between align-items-center font-size-16">
                                ...
                                <span class="fw-bold text-primary font-monospace fs-5">{{ currency() }} {{ number_format($grand_total, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
